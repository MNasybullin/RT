...
/*   Updated: 2020/10/08 13:32:35 by mgalt            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_obj_sphere(t_data *p, t_world *w, char **tab)
{
	...
	if (w->s[p->sp_i].pattern)
	{
		...
		if (w->s[p->sp_i].pattern == 1) // checker
			checker_pattern_sp(w->s[p->sp_i].m.p.a, w->s[p->sp_i].m.p.b, &w->s[p->sp_i]);
		else if (w->pl[p->pl_i].pattern == 2) // stripe
			stripe_pattern_sp(w->s[p->sp_i].m.p.a, w->s[p->sp_i].m.p.b, &w->s[p->sp_i]);
	}
