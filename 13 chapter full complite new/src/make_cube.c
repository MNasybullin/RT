...
void	make_obj_cube(t_data *p, t_world *w, char **tab)
{
	...
	if (w->cub[p->cube_i].pattern)
	{
		if (w->cub[p->cube_i].pattern == 1) // checker
			checker_pattern_cube(w->cub[p->cube_i].m.p.a, w->cub[p->cube_i].m.p.b, &w->cub[p->cube_i]);
		else if (w->cub[p->cube_i].pattern == 2) // stripe
			stripe_pattern_cube(w->cub[p->cube_i].m.p.a, w->cub[p->cube_i].m.p.b, &w->cub[p->cube_i]);
	}
}
