...
void	make_obj_tri(t_data *p, t_world *w, char **tab)
{
	...
	if (w->trian[p->tri_i].pattern)
	{
		if (w->trian[p->tri_i].pattern == 1) // checker
			checker_pattern_trian(w->trian[p->tri_i].m.p.a, w->trian[p->tri_i].m.p.b, &w->trian[p->tri_i]);
		else if (w->trian[p->tri_i].pattern == 2) // stripe
			stripe_pattern_trian(w->trian[p->tri_i].m.p.a, w->trian[p->tri_i].m.p.b, &w->trian[p->tri_i]);
	}
}
