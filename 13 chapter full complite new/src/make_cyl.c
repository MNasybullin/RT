...
/*   Updated: 2020/11/26 18:16:44 by mgalt            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_obj_cyl(t_data *p, t_world *w, char **tab)
{
	...
	if (w->cyl[p->cyl_i].pattern)
	{
		if (w->cyl[p->cyl_i].pattern == 1) // checker
			checker_pattern_cyl(w->cyl[p->cyl_i].m.p.a, w->cyl[p->cyl_i].m.p.b, &w->cyl[p->cyl_i]);
		else if (w->cyl[p->cyl_i].pattern == 2) // stripe
			stripe_pattern_cyl(w->cyl[p->cyl_i].m.p.a, w->cyl[p->cyl_i].m.p.b, &w->cyl[p->cyl_i]);
	}
}
