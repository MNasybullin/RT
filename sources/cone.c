...
/*   Updated: 2021/02/13 18:33:15 by sdiego           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		inter_cone_disc(t_disc *d, t_ray ray2, t_x_t *x, t_cone *s)
{
	...
	if (SDL_fabs(d->a) <= EPSILON && SDL_fabs(d->b) <= EPSILON)
	{
...
{
	d.t0 = ((-d.b - SDL_sqrt(d.disc)) / (2 * d.a));
	d.t1 = ((-d.b + SDL_sqrt(d.disc)) / (2 * d.a));
	if (d.t0 > d.t1)
	{
		d.temp = d.t0;
		d.t0 = d.t1;
...
void	intersect_caps_cone(t_cone *cone, t_ray r, t_x_t *x, int obj_n)
{
	...
	if (cone->closed == 0 || SDL_fabs(r.d.c[1]) <= EPSILON)
		return ;
